<div class='page-header'>
  <div class='page-header-title'>
    <h4>Report Customer Information Report
    </h4>
	 <span>Laporan Informasi Pelanggan
    </span>
  </div>
...
echo"
	<tr>
		...
		<td><a href='?n=rpcir&act=detail&id=&code=$i[codeCustCare]'><b>$i[codeCustCare]</b></a></td>
		<td>$i[companyName]</td>
		<td>$i[designCode]</td>
		<td>$i[GID]</td>
		<td align='center'>";
	...
	if($clo[statusKasus]=='Closed'){
		echo"<font color='black' size='2'><B><b>Closed</B></font>";
		...
		if($i[action]=='Forwarded'){
			...
			if(mysqli_num_rows($sq)>0 || mysqli_num_rows($sq2)>0){
				echo"<font color='green' size='2'><B>Replied</B></font>";
			}else{
				echo"<font color='red' size='2'><B>Forwarded</B></font>";
			}
			if(mysqli_num_rows($apr)>0){
				 }else{}
		}else{
			echo"<font color='blue' size='2'><B><b>Open</B></font>";
		}
	}
	echo"</td>
	";
	...
	echo "<td>";
			...
			while($d = mysqli_fetch_array($qs)){
				echo"<a href='?n=forward-cir&act=detail-jawab-segera&id=$i[idCirCust]&code=$i[codeCustCare]&no=$d[idCirIm]' title='$d[DepMain]'><button type='button' class='btn btn-primary btn-mini'><i class='icofont icofont-user-alt-3'></i></button></a>";
			}
			...
			while($ds = mysqli_fetch_array($qp)){
				echo"<a href='?n=forward-cir&act=detail-jawab-tindakan&id=$i[idCirCust]&code=$i[codeCustCare]&no=$ds[idCA]' title='$ds[DepMain]'><button type='button' class='btn btn-danger btn-mini'><i class='icofont icofont-user-alt-3'></i></button></a>";
			}
